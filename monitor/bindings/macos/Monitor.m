...
void start_mouse_monitoring(MouseEventCallback callback) {
  ...
  id clickMonitor = [NSEvent
      addGlobalMonitorForEventsMatchingMask:clickMask
                                    handler:^(NSEvent *event) {
...
void start_keyboard_monitoring(KeyboardEventCallback callback) {
  ...
  if (keyboardMonitor) {
    ...
    monitorHolder.monitors =
        [monitorHolder.monitors arrayByAddingObject:keyboardMonitor];
  }
}
...
void process_events(void) {
  @try {
    ...
    // printf("Until: %p\n", until);
    NSDate *until =
        [NSDate dateWithTimeIntervalSinceNow:0.1]; // 100ms collection window
    NSEvent *event;
