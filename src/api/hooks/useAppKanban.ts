// hooks/useAppKanbanData.ts
import { useState, useEffect, useRef } from 'react';
import { DragEndEvent, DragStartEvent } from '@dnd-kit/core';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'; // NEW: Import Tanstack Query hooks
import { MonitorApi, AppsWithTime } from '@/api/monitorApi/monitorApi'; // Adjust path as needed for types
...
// export const useAppKanbanData = ({ appUsage, onRatingChange, tags = [] }: UseAppKanbanDataProps) => { // OLD
export const useAppKanbanData = () => { // NEW: No props needed for the hook
  const queryClient = useQueryClient(); // NEW: Initialize query client

  // NEW: Fetch app usage data using useQuery
  const { data: fetchedAppUsage, isLoading: isLoadingAppUsage, error: appUsageError } = useQuery<AppsWithTime[]>({
    queryKey: appKanbanKeys.appUsage(),
    queryFn: () => MonitorApi.getTopAppsByPeriod(DateTime.local().startOf('day'), DateTime.local().endOf('day')),
    staleTime: 5 * 60 * 1000, // Data considered fresh for 5 minutes
    refetchOnWindowFocus: false, // Prevent refetch on window focus for this data
  });

  // NEW: Fetch tags data using useQuery
  const { data: fetchedTags, isLoading: isLoadingTags, error: tagsError } = useQuery<Tag[]>({
  ...
  // NEW: Mutation for updating app rating
  const updateAppRatingMutation = useMutation({
  ...
  useEffect(() => {
    ...
    if (fetchedAppUsage && fetchedAppUsage.length > 0 && !isInitialized.current) {
      console.log('useAppKanbanData: Initializing columns from fetchedAppUsage:', fetchedAppUsage);
      ...
      isInitialized.current = true;
       ...
       console.log('useAppKanbanData: fetchedAppUsage changed but columns already initialized. Not re-initializing.');
